...
//-----------------------------------------------------------------------------
// global variable(s)
volatile char     sx128x_hw_irq_state = 0; // GPIO IRQ state (DIO1)
volatile char     sx128x_hw_irq_flag  = 0; // interrupt flag by DIO1
volatile unsigned sx128x_hw_irq_cnt   = 0; // interrupt counter
volatile unsigned sx128x_hw_irq_ms    = 0; // time of last interrupt [ms]
//-----------------------------------------------------------------------------
// init SPI/GPIO
void sx128x_hw_begin()
{
  ...
  pinMode(SX128X_DIO1_PIN, INPUT); // interrupt by periodic read
...
{ 
  ...
  if (state == 1 && sx128x_hw_irq_state == 0)
  { // IRQ DIO1 rise
    sx128x_hw_irq_ms = t;
    sx128x_hw_irq_cnt++;
    sx128x_hw_irq_flag = 1;
  }
  sx128x_hw_irq_state = state;
}
//-----------------------------------------------------------------------------
